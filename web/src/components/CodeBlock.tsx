...
function highlightDryLang(code: string) {
  ...
  const regex = /(\/\/.*)|(".*?")|(\b(?:fn|if|el|elif|lp|pt|cns|rev|brk|ret|imp|len|push|pop|del|map|keys)\b)|(\b\d+(?:,\d+)?\b)|(\b(?:tr|fs)\b)|(\b[a-zA-Z_]\w*\b(?=\())/g;
  ...
  return html.replace(regex, (match, p1, p2, p3, p4, p5, p6) => {
    ...
    if (p3) return `<span style="color: #c586c0;">${p3}</span>`;
    if (p4) return `<span style="color: #b5cea8;">${p4}</span>`;
    if (p5) return `<span style="color: #569cd6;">${p5}</span>`;
    if (p6) return `<span style="color: #dcdcaa;">${p6}</span>`;
    return match;
  });
}
...
export function CodeBlock({ node, className, children, ...props }: any) {
  ...
  return (
    <div style={{ position: 'relative', margin: '1.5rem 0', borderRadius: '0', background: 'transparent', border: '1px solid var(--border-color)', overflow: 'hidden' }}>
      <div style={{
        position: 'absolute',
        top: 0,
        right: 0,
