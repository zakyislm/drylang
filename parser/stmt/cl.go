...
func ParseClass(p core.ParserCore) (ast.Stmt, error) {
	...
	for p.Current().Type != lexer.TOKEN_RBRACE && p.Current().Type != lexer.TOKEN_EOF {
		...
		if p.Current().Type == lexer.TOKEN_IDENT {
			...
			if vis == "pv" {
				privateFields = append(privateFields, p.Current().Literal)
			}
			p.Advance()
	...
	return &ast.ClassStmt{Name: name.Literal, Extends: extends, Fields: fields, PrivateFields: privateFields, Methods: methods, Line: tok.Line, Col: tok.Col}, nil
}
